...
from .models import (
    UserProfile, FollowDataUser, User_User, UserUpdateForm,
    getunbr_user, update_user_profile, update_Last_Prediction_text,
    Image, Service
)

ML_API_URL = os.environ.get('ML_API_URL', 'http://ml_api:5000')
...
class UserProfileUpdateForm(forms.ModelForm):
    ...
    YES_NO_MAYBE_CHOICES = [('Yes', 'Yes'), ('No', 'No'), ('Maybe', 'Maybe')]
    ...
    class Meta:
        model = UserProfile
        fields = [
            'age', 'sexe', 'height', 'weight', 'steps', 'sleep_quality',
            'sleep_duration', 'physical_activity', 'stress_level',
            'ch2o','fcvc','ncp','Days_Indoors', 'Changes_Habits',
            'Work_Interest', 'Social_Weakness', 'Mental_Health_History'
        ]

# ----------------- API ML Call Helper -----------------
def call_ml_api(endpoint, user_data):
    try:
        response = requests.post(f"{ML_API_URL}/{endpoint}", json=user_data)
        if response.status_code == 200:
            return response.json().get('prediction')
        else:
            print(f"Erreur API {endpoint}: {response.text}")
            return None
    except Exception as e:
        print(f"Erreur appel API ML ({endpoint}): {e}")
        return None

# ----------------- Vues principales -----------------
def hello_world(request):
    return render(request, 'index.html', {'user_count': getunbr_user()})
...
def user_profile(request):
    ...
    return render(request, 'profile.html', {'user': request.user, 'profile': profile, 'history': history})

@login_required
def update_profile(request):
    profile = UserProfile.objects.get(user=request.user)
    form = UserProfileUpdateForm(request.POST or None, instance=profile)
    if form.is_valid():
        form.save()
...
def predict_sleep_disorder_view(request):
    profile = UserProfile.objects.get(user=request.user)
    user_data = {
        'age': profile.age, 'sexe': profile.sexe, 'height': profile.height, 'weight': profile.weight,
        'steps': profile.steps, 'sleep_quality': profile.sleep_quality, 'sleep_duration': profile.sleep_duration,
        'physical_activity': profile.physical_activity, 'stress_level': profile.stress_level,
        'ch2o': profile.ch2o, 'fcvc': profile.fcvc, 'ncp': profile.ncp,
        'Days_Indoors': profile.Days_Indoors, 'Changes_Habits': profile.Changes_Habits,
        'Work_Interest': profile.Work_Interest, 'Social_Weakness': profile.Social_Weakness,
        'Mental_Health_History': profile.Mental_Health_History
    }

    sleep_prediction = call_ml_api('predict_sleep', user_data)
    obesity_prediction = call_ml_api('predict_obesity', user_data)
    stress_prediction = call_ml_api('predict_stress', user_data)

    sleep_msg = sleep_disorder_messages.get(sleep_prediction, "Donnée non disponible")
    obesity_msg = obesity_status_messages.get(obesity_prediction, "Donnée non disponible")
    stress_msg = stress_status_messages.get(stress_prediction, "Donnée non disponible")

    recommendations = '\n'.join([
        *provide_detailed_recommendations_obesity(user_data),
        *provide_detailed_recommendations_stress(user_data),
        *provide_detailed_recommendations_nutrition(user_data)
    ])

    update_Last_Prediction_text(request.user.id, recommendations, sleep_msg, obesity_msg, stress_msg)

    return render(request, 'prediction_result.html', {
        'sleep_disorder_message': sleep_msg,
        'obesity_status_message': obesity_msg,
        'stress_status_message': stress_msg,
